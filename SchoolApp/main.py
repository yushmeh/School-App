...
def export_student(sid: str):
    return _csv_response(svc.export_student_csv(sid), f"student_{sid}.csv")

@app.get("/api/export/teacher/{tid}")
def export_teacher(tid: str):
    return _csv_response(svc.export_teacher_csv(tid), f"teacher_{tid}.csv")

@app.get("/api/export/course/{cid}")
def export_course(cid: str):
    return _csv_response(svc.export_course_csv(cid), f"course_{cid}.csv")


@app.post("/api/seed")
def force_seed():
    """Сбрасывает БД и заполняет тестовыми данными из seed.py."""
    repo.reset()
    run_seed(repo)
    return {"status": "ok"}